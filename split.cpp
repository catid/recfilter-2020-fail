...
struct SplitInfo {
    ...
    bool clamped_border;                ///< Buffer border expression (from RecFilterContents)
 ...
 */
Buffer<float> tail_weights(SplitInfo s, int split_id1, int split_id2, bool clamp_border=false) {
    ...
    Buffer<float> Z(tile_width, s.filter_order);
    ...
    // memory outside tile boundaries in the dimension found above
    {
        ...
        for (int i=0; i<split_info.size(); i++) {
            ...
            for (int j=0; j<split_info[i].num_scans; j++) {
                ...
                auto& scan_vals = updates[scan_id].values();
                for (int k=0; k<scan_vals.size(); k++) {
    ...
    // above created function, the tail functions were originally left undef
    {
        ...
        for (int l=0; l<split_info.size(); l++) {
            ...
            for (int k=0; k<split_info[l].num_scans; k++) {
                ...
                ftail = Function(ftail.name());
                rF_tail[l][k].func = ftail;
...
static RecFilterFunc create_copy(RecFilterFunc rF, string func_name) {
    ...
    for (int i=0; i<F.updates().size(); i++) {
        ...
        for (int j=0; j<rargs.size(); j++) {
            args.push_back(substitute_func_call(F.name(), B, rargs[j]));
        }
        auto& rvals = r.args();
        for (int j=0; j<rvals.size(); j++) {
            values.push_back(substitute_func_call(F.name(), B, rvals[j]));
        }
        ...
        string func_name)
{
    ...
    for (int k=0; k<split_info.num_scans; k++) {
        ...
        Buffer<float> weight = tail_weights(split_info, k);
        ...
        string final_result_func)
{
    ...
    for (int j=0; j<split_info.num_scans; j++) {
        ...
        // Buffer<float> weight = tail_weights(split_info, j);
        ...
        SplitInfo split_info)
{
    ...
    for (int j=0; j<split_info.num_scans-1; j++) {
        ...
        // redefine tail
        F_tail[j] = Function(F_tail[j].name());
    ...
    RecFilterFunc rF_reidx;
    {
        ...
        for (int i=0; i<intra_tile_funcs.size(); i++) {
            ...
            for (int k=0; k<updates.size(); k++) {
                auto& kvals = updates[k].values();
                for (int j=0; j<kvals.size(); j++) {
                    Expr v1  = substitute_func_call(f.name(), F, kvals[j]);
        ...
        for (int k=0; k<updates.size(); k++) {
            ...
            auto& kvals = updates[k].values();
            for (int j=0; j<kvals.size(); j++) {
                Expr v = insert_arg_in_func_call(F.name(), dimension, c, kvals[j]);
                kvals[j] = v;
            }
            F.define_update(updates[k].args(), kvals);
        }
        ...
        vector<SplitInfo> split_info)
{
    ...
    for (int i=0; i<F_deps.size(); i++) {
        ...
        for (int j=0; j<F_deps[i].size(); j++) {
            ...
            vector<Expr> values;
            auto &cvals = updates[curr_scan].values();
            for (int k=0; k<F_deps[i][j].outputs(); k++) {
                values.push_back(cvals[k] + Call::make(F_deps[i][j], args, k));
            }
            ...
            for (int k=0; k<rxi.dimensions(); k++) {
                ...
                auto& cargs = updates[curr_scan].args();
                for (int u=0; u<cargs.size(); u++) {
                    cargs[u] = substitute(rxi[k].name(), rxf[k], cargs[u]);
                }
                for (int u=0; u<cvals.size(); u++) {
                    cvals[u] = substitute(rxi[k].name(), rxf[k], cvals[u]);
                }
...
void RecFilter::split(map<string,int> dim_tile) {
    ...
    ptr->filter_info = group_scans_by_dimension(rF.func, ptr->filter_info);
    Function        F = rF.func;
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        if (dim_tile.find(ptr->filter_info[i].var.name()) != dim_tile.end()) {

            // check that there are scans in this dimension
            if (ptr->filter_info[i].scan_id.empty()) {
                cerr << "No scans to tile in dimension "
                    << ptr->filter_info[i].var.name() << endl;
                assert(false);
            }

            ptr->filter_info[i].tile_width = dim_tile[ptr->filter_info[i].var.name()];
        }

        Expr extent = 1;
        if (ptr->filter_info[i].tile_width != ptr->filter_info[i].image_width) {
            extent = ptr->filter_info[i].tile_width;
        }

        ReductionVariable r;
        r.min    = 0;
        r.extent = extent;
        r.var    = "r" + ptr->filter_info[i].var.name() + "i";
    ...
    for (map<string,int>::iterator it=dim_tile.begin(); it!=dim_tile.end(); it++) {
        ...
        for (int j=0; !found && j<ptr->filter_info.size(); j++) {
            if (x != ptr->filter_info[j].var.name()) {
                continue;
            }
            found = true;

            // no need to add a split if tile width is not same as image width
            assert(ptr->filter_info[j].tile_width == tile_width);
            if (ptr->filter_info[j].image_width == tile_width) {
                continue;
            }
            ...
            s.filter_order    = ptr->filter_info[j].filter_order;
            s.filter_dim      = ptr->filter_info[j].filter_dim;
            s.num_scans       = ptr->filter_info[j].num_scans;
            s.var             = ptr->filter_info[j].var;
            s.rdom            = ptr->filter_info[j].rdom;
            s.scan_causal     = ptr->filter_info[j].scan_causal;
            s.scan_id         = ptr->filter_info[j].scan_id;
            s.image_width     = ptr->filter_info[j].image_width;
            s.tile_width      = ptr->filter_info[j].tile_width;
            s.num_tiles       = ptr->filter_info[j].image_width / tile_width;

            s.feedfwd_coeff   = ptr->feedfwd_coeff;
            s.feedback_coeff  = ptr->feedback_coeff;
            s.clamped_border  = ptr->clamped_border;
            s.type            = ptr->type;
        ...
        if (!found) {
            cerr << "Variable " << x << " does not correspond to any "
                << "dimension of the recursive filter " << ptr->name << endl;
            assert(false);
        }
    }
    ...
    ptr->func.insert(recfilter_func_list.begin(), recfilter_func_list.end());
...
void RecFilter::split_all_dimensions(int tx) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        if (ptr->filter_info[i].num_scans) {
            dim_tile.insert(make_pair(ptr->filter_info[i].var.name(),tx));
        }
    }
    split(dim_tile);
}
...
void RecFilter::apply_bounds(void) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        string x = ptr->filter_info[i].var.name();
        int    w = ptr->filter_info[i].image_width;
    ...
    if (ptr->tiled) {
        ...
        for (fit=ptr->func.begin(); fit!=ptr->func.end(); fit++) {
            if (fit->second.func_category == INLINE) {
                ...
                fit = ptr->func.begin();            // list changed, start all over again
            }
        }
        ...
        for (fit=ptr->func.begin(); fit!=ptr->func.end(); fit++) {
            ...
            if (rF.func_category==INTRA_1 || rF.func_category==INTRA_N) {
                ...
                for (git=ptr->func.begin(); git!=ptr->func.end(); git++) {
