...
static Function add_pure_def_to_first_update_def(Function f) {
    ...
    auto& u0values = updates[0].values();
    auto& u0args = updates[0].args();

    // add pure def to the first update def
    for (int j=0; j<u0values.size(); j++) {
        // replace pure args by update def args in the pure value
        Expr val = values[j];
        for (int k=0; k<args.size(); k++) {
            val = substitute(args[k], u0args[k], val);
        }

        // remove let statements in the expression because we need to
        // compare calling args
        u0values[j] = remove_lets(u0values[j]);

        // remove call to current pixel of the function
        u0values[j] = substitute_func_call_with_args(f.name(),
                u0args, val, u0values[j]);
    }
    ...
    Function new_f(f.name());
    new_f.define(args, values);
    for (int i=0; i<updates.size(); i++) {
        new_f.define_update(updates[i].args(), updates[i].values());
    }
    return new_f;
}
...
RecFilterSchedule& RecFilterSchedule::compute_globally(void) {
    for (int j=0; j<func_list.size(); j++) {
        ...
        if (rF.func_category==INTER) {
            rF.func = add_pure_def_to_first_update_def(F.function());
            F = Func(rF.func);
        }
