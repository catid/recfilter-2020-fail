...
private:
    ...
    Expr visit(const Let *let) {
        Expr var = canonicalize(Variable::make(let->value.type(), let->name));

        Expr new_value = process_stuff(let->value);
        enter_scope();
        add_replacement(var, new_value);
        Expr expr = process_stuff(let->body);
        leave_scope();
...
public:
    ...
    // FIXME: How do we do this override properly?
    Expr process_stuff(Expr e) {
        e = canonicalize(e);
...
private:
    ...
    Expr visit(const Call *op) {
        ...
        {
            Function f(op->func);
            expr = make_zero(f.output_types()[op->value_index]);
        }
...
private:
    using IRMutator::visit;
    Expr visit(const Max *op) { return (is_const(op->a) ? op->b : op->a); }
    Expr visit(const Min *op) { return (is_const(op->a) ? op->b : op->a); }
public:
    RemoveMinMax(void) {}
};
...
private:
    ...
    Expr visit(const Call *op) {
        ...
        if (change) {
            return select(condition, op, make_zero(op->type));
        } else {
            return op;
        }
    }
...
Expr remove_lets(Expr e) {
    return RemoveLets().process_stuff(e);
}
...
Expr swap_vars_in_expr(string a, string b, Expr original) {
    Expr value = original;
    string t = "temp_var_" + std::to_string(rand());
...
void inline_function(Function f, vector<Func> func_list) {
    ...
    for (int j=0; j<func_list.size(); j++) {
        // FIXME: Are all the side-effects captured by this port?
        inline_function(func_list[j].function(), f);
#if 0
        ...
        vector<Definition> updates = g.updates();

        for (int k=0; k<values.size(); k++) {
            values[k] = inline_function(values[k], f);
        }

        Function new_g(g.name());
        new_g.define(args, values);
        ...
        for (int k=0; k<updates.size(); k++) {
            ...
            new_g.define_update(update_args, update_values);
        }
