...
template<typename T>
vector<T> extract_row(Buffer<T> img, int i) {
    vector<T> res;
...
vector<RecFilter> RecFilter::cascade(vector<vector<int> > scans) {
    ...
    if (ptr->tiled || ptr->compiled || ptr->finalized) {
    ...
    // check that the order does not violate
    {
        ...
        for (int i=0; i<ptr->filter_info.size(); i++) {
            int scan_dim = ptr->filter_info[i].filter_dim;
            for (int j=0; j<ptr->filter_info[i].num_scans; j++) {
                int scan_id = ptr->filter_info[i].scan_id[j];
                bool causal = ptr->filter_info[i].scan_causal[j];
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        args.push_back(RecFilterDim(
                    ptr->filter_info[i].var.name(),
                    ptr->filter_info[i].image_width));
    }

    vector<RecFilter> recfilters;

    for (int i=0; i<scans.size(); i++) {
        RecFilter rf(ptr->name + "_" + std::to_string(i));
        ...
        for (int j=0; j<scans[i].size(); j++) {
            ...
            int dim = -1;
            int idx = -1;
            for (int u=0; dim<0 && u<ptr->filter_info.size(); u++) {
                for (int v=0; idx<0 && v<ptr->filter_info[u].num_scans; v++) {
                    if (scan_id == ptr->filter_info[u].scan_id[v]) {
                        dim = u;
                        idx = v;
                    }
                }
            ...
            RecFilterDim x(ptr->filter_info[dim].var.name(),
                    ptr->filter_info[dim].image_width);
            bool causal = ptr->filter_info[dim].scan_causal[idx];
            int order   = ptr->filter_info[dim].filter_order;

            vector<float> coeff;
            coeff.push_back(ptr->feedfwd_coeff(scan_id));
            for (int u=0; u<order; u++) {
                coeff.push_back(ptr->feedback_coeff(scan_id,u));
            }
...
vector<RecFilter> RecFilter::cascade_by_causality(void) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        for (int j=ptr->filter_info[i].num_scans-1; j>=0; j--) {
            int id = ptr->filter_info[i].scan_id[j];
            bool c = ptr->filter_info[i].scan_causal[j];
            if (c) {
...
vector<RecFilter> RecFilter::cascade_by_dimension(void) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        ...
        for (int j=ptr->filter_info[i].num_scans-1; j>=0; j--) {
            dim_scans.push_back(ptr->filter_info[i].scan_id[j]);
        }
        if (!dim_scans.empty()) {
...
RecFilter RecFilter::overlap_to_higher_order_filter(RecFilter fB, string overlap_name) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        string var= ptr->filter_info[i].var.name();
        int width = ptr->filter_info[i].image_width;
        filter_dim.push_back(RecFilterDim(var, width));
    }

    // check that both filters have same border clamping
    bool border_clamp_a = ptr->clamped_border;
    bool border_clamp_b = fB.contents.get()->clamped_border;
    ...
    if (ptr->type != fB.contents.get()->type) {
    ...
    Buffer<float> feedfwd_a = ptr->feedfwd_coeff;
    Buffer<float> feedfwd_b = fB.contents.get()->feedfwd_coeff;
    Buffer<float> feedback_a= ptr->feedback_coeff;
    Buffer<float> feedback_b= fB.contents.get()->feedback_coeff;

    // add the scans
    for (int i=0; i<ptr->filter_info.size(); i++) {
        int  filter_dim_a   = ptr->filter_info[i].filter_dim;
        int  num_scans_a    = ptr->filter_info[i].num_scans;
        int  image_width_a  = ptr->filter_info[i].image_width;
        int  tile_width_a   = ptr->filter_info[i].tile_width;
        Var  var_a          = ptr->filter_info[i].var;
        RDom rdom_a         = ptr->filter_info[i].rdom;

        int  filter_dim_b   = fB.contents.get()->filter_info[i].filter_dim;
        int  num_scans_b    = fB.contents.get()->filter_info[i].num_scans;
        int  image_width_b  = fB.contents.get()->filter_info[i].image_width;
        int  tile_width_b   = fB.contents.get()->filter_info[i].tile_width;
        Var  var_b          = fB.contents.get()->filter_info[i].var;
        RDom rdom_b         = fB.contents.get()->filter_info[i].rdom;
        ...
        for (int j=0; j<num_scans_a; j++) {
            bool scan_causal_a = ptr->filter_info[i].scan_causal[j];
            int  scan_id_a     = ptr->filter_info[i].scan_id[j];
            bool scan_causal_b = fB.contents.get()->filter_info[i].scan_causal[j];
            int  scan_id_b     = fB.contents.get()->filter_info[i].scan_id[j];
