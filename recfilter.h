...
public:
    ...
    /** @name Buffer boundary conditions
...
template<typename T>
Halide::Buffer<T> generate_random_image(size_t w, size_t h=0, size_t c=0, size_t d=0) {
    Halide::Buffer<T> image;

    int MIN_ELEMENT = 1;
    int MAX_ELEMENT = 1;

    if (w && h && c && d) {
        image = Halide::Buffer<T>(w,h,c,d);
    } else if (w && h && c) {
        image = Halide::Buffer<T>(w,h,c);
    } else if (w && h) {
        image = Halide::Buffer<T>(w,h);
    } else if (w) {
        image = Halide::Buffer<T>(w);
    }
...
/** Print an image */
template<typename T>
std::ostream &operator<<(std::ostream &s, Halide::Buffer<T> image) {
    int precision = 4;
    if (image.dimensions() == 1) {
...
public:
    ...
    Halide::Buffer<T> ref;       ///< reference solution
    Halide::Buffer<T> out;       ///< Halide solution
    Halide::Buffer<float> diff;  ///< pixel wise diff

    CheckResult(Halide::Buffer<T> r, Halide::Buffer<T> o)
        : max_diff(0.0), mean_diff(0.0), ref(r), out(o)
    {
        ...
        diff = Halide::Buffer<float>(width, height, channels);
...
public:
    CheckResultVerbose(Halide::Buffer<T> r, Halide::Buffer<T> o) :
        CheckResult<T>(r,o) {}
};
