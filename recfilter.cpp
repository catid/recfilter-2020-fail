...
RecFilter::RecFilter(string name) {
    ...
    auto ptr = contents.get();
    if (name.empty()) {
        ptr->name = unique_name("R");
    } else {
        ptr->name = unique_name(name);
    }
    ptr->tiled          = false;
    ptr->finalized      = false;
    ptr->compiled       = false;
    ptr->clamped_border = false;
    ptr->feedfwd_coeff  = Buffer<float>(0);
    ptr->feedback_coeff = Buffer<float>(0,0);

    ptr->target = get_jit_target_from_environment();
    if (ptr->target.to_string().empty()) {
...
void RecFilter::define(vector<RecFilterDim> pure_args, vector<Expr> pure_def) {
    ...
    ptr->type = pure_def[0].type();
    for (int i=1; i<pure_def.size(); i++) {
        if (ptr->type != pure_def[i].type()) {
            cerr << "Type of all Tuple elements in filter definition must be same" << endl;
            assert(false);
        }
    }

    if (!ptr->filter_info.empty()) {
        cerr << "Recursive filter " << ptr->name << " already defined" << endl;
        assert(false);
    }

    RecFilterFunc rf;
    rf.func = Function(ptr->name);
    ...
    for (int i=0; i<pure_args.size(); i++) {
        ...
        ptr->filter_info.push_back(s);

        // add tag the dimension as pure
        rf.pure_var_category.insert(make_pair(pure_args[i].var().name(), VarTag(FULL,i)));
    }

    ptr->func.insert(make_pair(rf.func.name(), rf));

    // add the right hand side definition
    Function f = rf.func;

    vector<string> args;
    for (int i=0; i<ptr->filter_info.size(); i++) {
        args.push_back(ptr->filter_info[i].var.name());
    }
...
void RecFilter::add_filter(RecFilterDimAndCausality x, vector<float> coeff) {
    ...
    RecFilterFunc& rf = internal_function(ptr->name);
    Function        f = rf.func;
    ...
    RDom rx    = ptr->filter_info[dimension].rdom;
    Expr width = ptr->filter_info[dimension].image_width;
    ...
    for (int i=0; i<values.size(); i++) {
        values[i] = Cast::make(ptr->type,feedfwd) *
            Call::make(f, args, i);
        ...
        for (int j=0; j<feedback.size(); j++) {
            ...
            if (ptr->clamped_border) {
                values[i] += Cast::make(ptr->type,feedback[j]) *
                    Call::make(f,call_args,i);
            } else {
                values[i] += Cast::make(ptr->type,feedback[j]) *
                    select(rx>j, Call::make(f,call_args,i), make_zero(ptr->type));
            }
        }
    }
    ...
    FilterInfo s = ptr->filter_info[dimension];
    ...
    int max_order = ptr->feedback_coeff.height();
    Buffer<float> feedfwd_coeff(num_scans);
    Buffer<float> feedback_coeff(num_scans, std::max(max_order,scan_order));
    for (int j=0; j<num_scans-1; j++) {
        feedfwd_coeff(j) = ptr->feedfwd_coeff(j);
        for (int i=0; i<ptr->feedback_coeff.height(); i++) {
            feedback_coeff(j,i) = ptr->feedback_coeff(j,i);
        }
    }
    ...
    ptr->feedfwd_coeff  = feedfwd_coeff;
    ptr->feedback_coeff = feedback_coeff;
...
RecFilterSchedule RecFilter::intra_schedule(int id) {
    ...
    map<string,RecFilterFunc>::iterator f_it = ptr->func.begin();
    for (; f_it!=ptr->func.end(); f_it++) {
        ...
        if (function_condition) {
            ...
            map<string,RecFilterFunc>::iterator g_it = ptr->func.begin();
            for (; g_it!=ptr->func.end(); g_it++) {
...
void RecFilter::compute_at(RecFilter external) {
    ...
    // TBD: In release_2017_05_03 Halide removed gpu_blocks() and the changelog
    // indicates that it works more like tile so I'm assuming we can just drop
    // the GPU specialization that was here? -cat
    ...
    if (!found) {
        cerr << name() << " cannot be computed locally in another recursive "
            << "filter because the external filter has no tile index where "
...
void RecFilter::compute_at(Func external, Var looplevel) {
    ...
    if (ptr->func.find(external.name()) != ptr->func.end()) {
            ...
            !f.schedule().store_level().is_inlined())
    {
        cerr << "Cannot compute " << name() << " inside " << external.name()
            ...
            << f.schedule().compute_level().func() << " " << endl;
            // FIXME
//            << f.schedule().compute_level().var() << endl;
    ...
    if (!rF.producer_func.empty()) {
        ...
        if (!producer.func.schedule().compute_level().is_inlined() ||
                !producer.func.schedule().store_level().is_inlined())
        {
        ...
        for (fit=ptr->func.begin(); fit!=ptr->func.end(); fit++) {
...
void RecFilter::cpu_auto_intra_schedule(void) {
    ...
    int max_tile = 0;
    for (int i=0; i<ptr->filter_info.size(); i++) {
        if (ptr->filter_info[i].tile_width<ptr->filter_info[i].image_width) {
            max_tile = std::max(max_tile, ptr->filter_info[i].tile_width);
        }
    }
...
void RecFilter::gpu_auto_inter_schedule(void) {
    ...
    if (ptr->filter_info.size()>AUTO_SCHEDULE_MAX_DIMENSIONS) {
...
void RecFilter::gpu_auto_intra_schedule(int id) {
    ...
    for (int i=0; i<ptr->filter_info.size(); i++) {
        ...
        max_order  = std::max(max_order, ptr->filter_info[i].filter_order);
        num_scans += ptr->filter_info[i].num_scans;
    }
...
Func RecFilter::as_func(void) {
    ...
    if (ptr->func.empty()) {
        cerr << "Filter " << ptr->name << " not defined" << endl;
        assert(false);
    }
    return Func(internal_function(ptr->name).func);
}

Func RecFilter::func(string func_name) {
    auto ptr = contents.get();

    map<string,RecFilterFunc>::iterator f = ptr->func.find(func_name);
    if (f != ptr->func.end()) {
        return Func(f->second.func);
    } else {
        cerr << "Function " << func_name << " not found as a dependency of ";
        cerr << "recursive filter " << ptr->name << endl;
        assert(false);
    }
}
...
void RecFilter::compile_jit(string filename) {
    ...
    Func F = as_func();
    if (!filename.empty()) {
        // FIXME: Does this still emit HTML?
        F.compile_to_lowered_stmt(filename, F.infer_arguments());
    }
    F.compile_jit(ptr->target);
...
Realization RecFilter::create_realization(void) {
    ...
    Func F(internal_function(ptr->name).func);
    if (ptr->target.has_gpu_feature()) {
        // FIXME: Do we really need to copy buffers manually here?
    }

    // allocate the buffer
    vector<int> buffer_size;
    for (int i=0; i<ptr->filter_info.size(); i++) {
        buffer_size.push_back(ptr->filter_info[i].image_width);
    }

    // create a realization object
    vector<Buffer<>> buffers;
    for (int i=0; i<F.outputs(); i++) {
        buffers.push_back(Buffer<>(ptr->type, buffer_size));
    }
...
float RecFilter::profile(int iterations) {
    ...
    if (ptr->target.has_gpu_feature()) {
        F.realize(R, ptr->target); // warmup run
...
string RecFilter::print_synopsis(void) const {
    ...
    for (f=ptr->func.begin(); f!=ptr->func.end(); f++) {
        s << f->second << "\n";
    }
    s << "\n";
    return s.str();
...
void RecFilter::inline_func(string func_name) {
    ...
    if (ptr->name == func_name) {
        return;
    }

    // all the functions in this recfilter
    vector<Func> func_list;
    map<string,RecFilterFunc>::iterator f = ptr->func.begin();
    map<string,RecFilterFunc>::iterator fe= ptr->func.end();
    while (f!=fe) {
